...
def images_to_video(image_dir: str, output_path: str, fps: int = 30, duration: list = [1], threshold_files: list = [float('inf')], slow_episodes=None):
        ...
        if slow_episodes is None:
            ...
            if flag_slow:          
                for _ in range(n_frames):
                    writer.write(img)
                flag_slow = False
            elif int(num_epochs(fname)) in slow_episodes:
                flag_slow = True
                writer.write(img)
            else:
                writer.write(img)
...
if __name__ == "__main__":
    ...
    #episodios vs scores video
    carpeta = f"Desarrollo/Documentacion/actor/train_set/plot/"
    salida  = f"Desarrollo/Documentacion/videos/scores_vs_episodes.mp4"
    images_to_video(carpeta, salida, fps=1003/10, duration=[1], slow_episodes=[1500,4000,6500,25000])