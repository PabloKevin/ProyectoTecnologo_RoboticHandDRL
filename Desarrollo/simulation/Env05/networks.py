...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, hidden_layers=[64,32,32], name='critic', checkpoint_dir='Desarrollo/simulation/Env05/tmp/td3', 
                 learning_rate=0.001, rnn_hidden_size=32, rnn_layers=1):
        ...
        self.rnn = nn.GRU(hidden_layers[0], rnn_hidden_size, rnn_layers, batch_first=True)
        self.fc2 = nn.Linear(rnn_hidden_size, hidden_layers[1])
        self.fc3 = nn.Linear(hidden_layers[1], hidden_layers[2])
        self.fc4 = nn.Linear(hidden_layers[2], 1)

        self.optimizer = optim.AdamW(self.parameters(), lr=learning_rate)
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        print(f"Created Critic Network with RNN on device: {self.device}")
        self.to(self.device)

    def forward(self, state, action):
        # state: [batch, seq, state_dim], action: [batch, seq, action_dim]
        x = torch.cat([state, action], dim=-1)  # [batch, seq, state+action]
        x = F.leaky_relu(self.fc1(x))
        x, _ = self.rnn(x)  # [batch, seq, rnn_hidden_size]
        x = F.leaky_relu(self.fc2(x))
        x = F.leaky_relu(self.fc3(x))
        q_value = self.fc4(x)  # [batch, seq, 1]
        return q_value
...
class ActorNetwork(nn.Module):
    def __init__(self, input_dims=11, n_actions=1, hidden_layers=[64,32,32], name='actor', checkpoint_dir='Desarrollo/simulation/Env05/tmp/td3', learning_rate=0.001, rnn_hidden_size=32, rnn_layers=1):
        ...
        self.optimizer = optim.AdamW(self.parameters(), lr=learning_rate)
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        print(f"Created Actor Network with RNN on device: {self.device}")
        self.to(self.device)

    def forward(self, state):
        # state: [batch, seq, state_dim]
        x = F.leaky_relu(self.fc1(state))
        x, _ = self.rnn(x)  # [batch, seq, rnn_hidden_size]
        x = F.leaky_relu(self.fc2(x))
        x = F.leaky_relu(self.fc3(x))
        action = torch.tanh(self.fc4(x))  # [batch, seq, n_actions]
        return action
...
class ObserverNetwork(nn.Module):
    ...
    def __init__(self, 
                 ...
                 name="observer_best_test_logits_2", checkpoint_dir='Desarrollo/simulation/Env05/tmp/observer'):
