...
class Agent:
    def __init__(self, env, actor_learning_rate=0.003, critic_learning_rate=0.0008, tau=0.005, gamma=0.99, update_actor_interval=2, warmup=1000, 
                 max_size=1000000, hidden_layers=[64,32], batch_size=64, noise=0.1, checkpoint_dir='Desarrollo/simulation/Env05/tmp/td3', seq_len=5):
        ...
        self.memory = ReplayBuffer(max_size, self.input_dims, self.n_actions, seq_len=self.seq_len)
    ...
    def choose_action(self, obs_seq, validation=False):
        # obs_seq: [seq_len, input_dims]
        obs = T.tensor(obs_seq, dtype=T.float).unsqueeze(0).to(self.actor.device)  # [1, seq_len, input_dims]
        mu = self.actor(obs)  # [1, seq_len, n_actions]
        mu = mu[0, -1]  # Take the last action in the sequence
    ...
    def learn(self):
        ...
        # Convert to tensors
        state = T.tensor(state, dtype=T.float).to(self.critic_1.device)         # [batch, seq_len, state_dim]
        action = T.tensor(action, dtype=T.float).to(self.critic_1.device)       # [batch, seq_len, action_dim]
        reward = T.tensor(reward, dtype=T.float).to(self.critic_1.device)       # [batch, seq_len]
        next_state = T.tensor(next_state, dtype=T.float).to(self.critic_1.device)
        done = T.tensor(done, dtype=T.float).to(self.critic_1.device)           # [batch, seq_len]

        # Target actions for next_state
        target_actions = self.target_actor(next_state)                          # [batch, seq_len, n_actions]
        ...
        # Q-values for next_state and current state
        next_q1 = self.target_critic_1(next_state, target_actions)              # [batch, seq_len, 1]
        next_q2 = self.target_critic_2(next_state, target_actions)
        q1 = self.critic_1(state, action)
        q2 = self.critic_2(state, action)

        # Use only the last step in the sequence
        reward_last = reward[:, -1]                                             # [batch]
        done_last = done[:, -1]                                                 # [batch]
        next_q1_last = next_q1[:, -1, 0]                                        # [batch]
        next_q2_last = next_q2[:, -1, 0]
        q1_last = q1[:, -1, 0]
        q2_last = q2[:, -1, 0]

        next_q1_last[done_last > 0.5] = 0.0
        next_q2_last[done_last > 0.5] = 0.0

        next_critic_value = T.min(next_q1_last, next_q2_last)
        target = reward_last + self.gamma * next_critic_value * (1 - done_last)
        target = target.view(self.batch_size, 1)

        self.critic_1.optimizer.zero_grad()
        self.critic_2.optimizer.zero_grad()
        q1_loss = F.mse_loss(q1_last.view(self.batch_size, 1), target)
        q2_loss = F.mse_loss(q2_last.view(self.batch_size, 1), target)
        ...
        actor_loss = -T.mean(actor_q1_loss[:, -1, 0])  # Only last step
        actor_loss.backward()
