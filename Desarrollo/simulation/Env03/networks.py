...
class CriticNetwork(nn.Module):
    ...
    def __init__(self, input_dims, n_actions, conv_channels=[16, 32, 64], hidden_size=256, name='critic', checkpoint_dir='Desarrollo/simulation/Env03/tmp/td3', 
        ...
        self.fc4 = nn.Linear(1 + n_actions * n_choices_per_finger, 1)
    ...
    def forward(self, state, action_probs):
        ...
        q_value = torch.clamp(self.fc4(x), max=5.0)
...
class ActorNetwork(nn.Module):
    ...
    def __init__(self, input_dims, n_actions, n_choices_per_finger, conv_channels=[16, 32, 64], hidden_size=256, name='actor', checkpoint_dir='Desarrollo/simulation/Env03/tmp/td3', learning_rate=0.001):
        ...
        self.fc2 = nn.Linear(hidden_size, 5)
        self.fc3 = nn.Linear(5, 1)
        self.fc4 = nn.Linear(1, n_actions * n_choices_per_finger)
    ...
    def forward(self, state):
        ...
        if len(x.shape) == 4:  # Batch case: [batch_size, channels, height, width]
            ...
            x = F.leaky_relu(self.fc2(x))
            x = F.leaky_relu(self.fc3(x))
            logits = self.fc4(x).reshape((x.size(0),self.n_actions, self.n_choices_per_finger))
            ...
            x = F.leaky_relu(self.fc2(x))
            x = F.leaky_relu(self.fc3(x))
            logits = F.leaky_relu(self.fc4(x)).reshape((self.n_actions, self.n_choices_per_finger))
